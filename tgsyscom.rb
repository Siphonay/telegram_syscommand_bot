...
tgsyscom.get_updates(fail_silently: true) do |message|
  message.reply do |reply|
    reply.text = `#{ARGV[1]} #{message.text.sub(/(((?<!\\)(<|>|\|))|[0-9]+(?=(?<!\\)(<|>|\|)))(.*)/, '')} 2>&1` # Reply with the output of the command specified in the argument, with the sent message as parameter, and remove any redirection.
